...
if (!empty($search)) {
    ...
    $where1 .= " AND fullname LIKE '%$search%' OR zipcode LIKE '%$search%' OR id LIKE '%$search%'";
    $where2 .= " AND customer_name LIKE '%$search%' OR zipcode LIKE '%$search%' OR order_id LIKE '%$search%'";
}
...
(
    SELECT
        ...
        city,
        zipcode,
        tracking_docket as tracking_number
    FROM orderr
    $where1
)
...
foreach ($rows as $row) {
    ...
    if($row['tracking_number']){
        $statusBadge = '<span class="badge badge-light-success fw-bold px-4 py-3">Fullfilled</span>';
    }else{
        $statusBadge = '<span class="badge badge-light-warning fw-bold px-4 py-3">UnFullfilled</span>';
    }
    ...
    $product_details_query = mysqli_query($con,"SELECT SUM(product_qty) AS allcount FROM shopify_order_product WHERE orderr_id = '{$row['invoice_no']}'");
    ...
    $actions = '<a href="#" class="btn btn-sm btn-light btn-flex btn-center btn-active-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Actions 
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                            <div class="menu-item px-3">
                                <a href="'.$site_path.'/edit-order?id='.my_simple_crypt($row['invoice_no'],'encrypt_1').'" class="menu-link px-3">Edit Order</a>
                            </div>
                        </div>';
    $data[] = [
