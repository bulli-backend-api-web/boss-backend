...
/* 🔍 Search */
if (!empty($search)) {
    $where .= " AND material_name LIKE '%$search%'";
}

$stmtTotal = $con->prepare("SELECT COUNT(*) as total FROM material_master");
$stmtTotal->execute();
...
$stmtFiltered = $con->prepare("SELECT COUNT(*) as total FROM material_master $where");
...
/* 📦 MAIN DATA QUERY */
$sql = "SELECT id,material_name,unit,status FROM material_master  $where ORDER BY $orderColumn $orderDir LIMIT $start, $length";
