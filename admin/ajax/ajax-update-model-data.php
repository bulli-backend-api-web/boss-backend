...
$p       = $_POST;
$model_id = (int)($p['hidden_id'] ?? 0);
$measurement_id = (int)($p['measurement_id'] ?? 0);
...
$v_selected_tags       = clean($p['selected_tags'] ?? null);
...
$modelTypes = 'ssssisssssssssssssssssssssddssssssssi';
...
$sqlMeas = "UPDATE model_measurements SET
    measurement_unit=?,
    neck=?,
    bust_chest=?,
    under_bust=?,
...
$measTypes = 's' . str_repeat('d', 23) . 'ssi';
mysqli_stmt_bind_param(
    $stmtMeas,
    $measTypes,
    $v_measurement_unit,
...
if (!empty($_FILES['qrFile']['name'])) {
        $tmp_name = $_FILES['qrFile']['tmp_name'];
        $error    = $_FILES['qrFile']['error'];
        $file_name    = $_FILES['qrFile']['name'];
        if ($error == 0) {
            $extension = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
            $new_name = "qrcode_".time().'.'.$extension;
            move_uploaded_file($tmp_name,'../../uploads/model/'.$new_name);

            $update_query = "UPDATE models SET qr_code_image = ? where id = ?";
            $stmt2 = $con->prepare($update_query);
            $stmt2->bind_param("si",$new_name,$model_id);
            $stmt2->execute();
        }
}
jsonResponse(true, 'Model update successfully.', [
    'status'             => "success", 
    'model_row_id'       => $measurement_id,
    'model_id'           => $model_id,
]);