...
if (!empty($search)) {
    ...
    $where1 .= " AND fullname LIKE '%$search%' OR zipcode LIKE '%$search%'";
    $where2 .= " AND customer_name LIKE '%$search%' OR zipcode LIKE '%$search%'";
}
...
foreach ($rows as $row) {
    ...
    if ($row['status'] == 1) {
        ...
        $statusBadge = '<span class="badge badge-light-danger fw-bold px-4 py-3">Dispatch</span>';
    } elseif ($row['status'] == 4) {
        $statusBadge = '<span class="badge badge-light-danger fw-bold px-4 py-3">Reject</span>';
    } else {
