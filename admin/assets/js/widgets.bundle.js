    ...
    init: function() {
        !(function() {
            ...
            if (e) {
                ...
                // ✅ Fetch dynamic sales data via AJAX
                $.ajax({
                    url: hostUrl+"ajax/ajax-fetch-average-daily-sales",
                    method: "GET",
                    dataType: "json",
                    success: function(response) {
                        ...
                        // ✅ Build chart dynamically after data fetch
...
var KTChartsWidget3 = function() {
    ...
    var loadSalesChart = function(e) {
        $.ajax({
            url: hostUrl+"ajax/ajax-fetch-monthy-sales",
            method: "GET",
            dataType: "json",
            success: function(response) {
                var dates = response.map(item => item.date);
                var sales = response.map(item => parseFloat(item.sales.replace(/,/g, '')));

                var chartEl = document.getElementById("kt_charts_widget_3");
                if (!chartEl) return;

                var height = parseInt(KTUtil.css(chartEl, "height"));
                var colorGray = KTUtil.getCssVariableValue("--bs-gray-500");
                var colorBorder = KTUtil.getCssVariableValue("--bs-border-dashed-color");
                var colorSuccess = KTUtil.getCssVariableValue("--bs-primary");
                ...
                if (e.self) e.self.destroy();
                e.self = new ApexCharts(chartEl, options);
                e.self.render();
                e.rendered = !0;
            }
        });
...
var KTChartsWidget4 = function () {
    ...
    var t = function () {
        ...
        // AJAX Call
        $.ajax({
            url: hostUrl+"ajax/ajax-fetch-monthly-order-count",
            type: "GET",
            dataType: "json",
            success: function (response) {
                var a = parseInt(KTUtil.css(t, "height")),
                ...
                e.self = new ApexCharts(t, options);
                setTimeout(function () {
                    e.self.render();
                    e.rendered = !0;
                }, 200);
...
var KTChartsWidget446 = function () {
    var e = { self: null, rendered: !1 };

    var t = function () {
        var t = document.getElementById("kt_charts_widget_446");
        if (!t) return;

        // AJAX Call
        $.ajax({
            url: hostUrl+"ajax/ajax-citywise-customer-list",
            type: "GET",
            dataType: "json",
            success: function (response) {
                var a = parseInt(KTUtil.css(t, "height")),
