            ...
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                    <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                        <h1 class="page-heading d-flex flex-column justify-content-center text-gray-900 fw-bold fs-3 m-0">Rejected Order List</h1>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <table class="table align-middle table-row-dashed table-row-gray-300 gy-5 gs-7" id="kt_customers_table">
                                <thead  class="bg-light border-bottom">
                                    <tr class="text-gray-700 fw-bold fs-7">
                                        ...
                                        <th class="min-w-110px">Product</th>
                                        <th class="min-w-110px">Size</th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        var table = $('#kt_customers_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '<?php echo $site_path ?>/ajax/fetch-rejected-orders',
                type: 'POST',
                data: function (d) {
                    d.ajax = 1;
        ...
        $("#export_b2c_order_csv").on('click', function () {
            ...
            var search_by_channel = $("#search_by_channel").val();
            var search_by_order = $("#search_by_order").val();
            var $btn = $(this);
            $btn.prop('disabled', true)
                    .html('<span class="spinner-border spinner-border-sm align-middle me-2"></span>Generating report...');

            $.ajax({
                url: '<?php echo $site_path ?>/ajax/pending-order-export-csv',
                type: 'POST',
                data: {'from_date': from_date, 'to_date': to_date, 'search_by_channel': search_by_channel, 'search_by_order': search_by_order},
