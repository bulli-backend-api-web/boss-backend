...
$active_task = $pending_approval = $overdue = 0;

$active_task_sql = "SELECT count(id) as total_count from design";
$active_task_res = $con->query($active_task_sql);
if($active_task_res && $active_task_res->num_rows > 0){
    $active_task_row = $active_task_res->fetch_assoc();
    $active_task = $active_task_row['total_count'];
}

$pending_approval_sql = "SELECT count(id) as total_count from design where status = 0 and image is not null";
$pending_approval_res = $con->query($pending_approval_sql);
if($pending_approval_res && $pending_approval_res->num_rows > 0){
    $pending_approval_row = $pending_approval_res->fetch_assoc();
    $pending_approval = $pending_approval_row['total_count'];
}
?>
                ...
                <div class="card card-flush shadow-sm">
                    <div class="card mb-5">
                        <div class="card-body pt-5">
                            ...
                            <div class="d-flex flex-nowrap overflow-auto gap-5 pb-3">

                                <!-- Item -->
                                <div class="min-w-175px p-5 rounded border border-gray-300 bg-light">
                                    <div class="text-gray-700 fw-semibold fs-6">Active Task</div>
                                    <div class="fs-2hx fw-bold text-dark mt-2"><?= $active_task ?? 0 ?></div>
                                </div>

                                <div class="min-w-175px p-5 rounded border border-gray-300 bg-light">
                                    <div class="text-gray-700 fw-semibold fs-6">Pending Approval</div>
                                    <div class="fs-2hx fw-bold text-dark mt-2"><?= $pending_approval ?? 0 ?></div>
                                </div>

                                <div class="min-w-175px p-5 rounded border border-gray-300 bg-light">
                                    <div class="text-gray-700 fw-semibold fs-6">Overdue</div>
                                    <div class="fs-2hx fw-bold text-dark mt-2"><?= $overdue ?? 0 ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        ...
                        <a href="<?php echo $site_path; ?>/create-design" class="btn btn-primary"><i class="fa fa-plus"></i>Create Design</a>
                    </div>
